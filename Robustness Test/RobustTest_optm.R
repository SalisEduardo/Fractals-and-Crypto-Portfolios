...
ROI_maxSR <- execute_backtest_maxSR_optimizor("ROI")
DEoptim_maxSR <- execute_backtest_maxSR_optimizor("DEoptim")
strats_all_optm <- c(ROI_maxSR,DEoptim_maxSR)


ROI_maxSR[[1]]$name
DEoptim_maxSR[[1]]$name

roi_maxSR_KPIS <- get.RoubtsTest.KPIs(ROI_maxSR,name_pattern = '_')
deoptim_maxSR_KPIS <- get.RoubtsTest.KPIs(DEoptim_maxSR,name_pattern = '_')



for(y in c(2020,2021,2022)){
  path <- paste("Results/RobustTest_optm/optm_backtest",as.character(y) ,".csv",sep='')
...
path_weights <- "Results/RobustTest_optm/Weights/"
for(s in ROI_maxSR){
  name_strat <- paste(s$test,s$name,sep='_')
  name_strat <- str_replace(name_strat , "first_", "")
  name_strat <- str_replace(name_strat , "second_", "")
  name_strat <- str_replace(name_strat , "third_", "")
  
  
  dfw <- s$w %>% as.data.frame()
  rownames(dfw) <- name_strat
  path_dfw <- paste(path_weights,name_strat,".csv",sep='')
  dfw %>%  write.csv(path_dfw)
  
  
}

for(s in DEoptim_maxSR){
  name_strat <- paste(s$test,s$name,sep='_')
  name_strat <- str_replace(name_strat , "first_", "")
  name_strat <- str_replace(name_strat , "second_", "")
  name_strat <- str_replace(name_strat , "third_", "")
